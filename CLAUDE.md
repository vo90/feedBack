...
- `resolve_directory_grant(grant, purpose, *, consume=True)` — redeem an opaque, short-lived directory selection minted by the managed Desktop. The loader binds redemption to the current plugin id, so plugins cannot provide or forge `owner`; a purpose mismatch returns `None` without consuming the token. See `docs/plugin-directory-grants.md`.
