...
const QUALITY_SUFFIX = { maj: '', min: 'm', min7: 'm7', min6: 'm6', min9: 'm9', minmaj7: 'm(maj7)', hdim7: 'm7b5' };
...
function prepareChord(event) {
    ...
    const pitchClasses = rootPc === null ? [] : (intervals || [0]).map(n => mod12(rootPc + n));
    ...
    return {
        ...
        label: rootPc === null ? 'Unknown' : `${root}${suffix}${bassPc !== null ? '/' + cleanNote(event.bass) : ''}`,
        status: rootPc === null ? 'unknown' : !quality ? 'note' : 'chord',
        supported: rootPc !== null && !!intervals,
    };
}

/** Overrides replace the specified track (even [] clears it); absent tracks use pack data. */
export function createHarmonyTimeline({ keys = [], harmony = [], overrides = null } = {}) {
    const overrideKeys = overrides && Object.prototype.hasOwnProperty.call(overrides, 'keys');
    const overrideHarmony = overrides && Object.prototype.hasOwnProperty.call(overrides, 'harmony');
    return {
        keys: normalizeEvents(overrideKeys ? overrides.keys : keys, overrideKeys ? 'local' : 'feedpak', prepareKey),
        harmony: normalizeEvents(overrideHarmony ? overrides.harmony : harmony, overrideHarmony ? 'local' : 'feedpak', prepareChord),
    };
}
...
export function resolveHarmony(timeline, time, { upcomingCount = 4, loop = null } = {}) {
    ...
    let scale = keyEvent?.scale || null;
    let scaleStatus = keyEvent?.scaleStatus || 'missing';
    ...
    return {
        ...
        source: { key: keyEvent?.source || null, harmony: current?.source || null },
    };
}
...
export function buildScaleFretboard({ scale, targetPc = null, openMidi = [], minFret = 0, maxFret = 24, focusFret = 5 } = {}) {
    ...
    const base = mod12(shapeTonic - openMidi[0]);
    const positions = [];
