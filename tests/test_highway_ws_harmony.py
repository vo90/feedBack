...
def test_optional_pitch_metadata_failure_does_not_prevent_playback(tmp_path, stream, monkeypatch):
    def unavailable(*args):
        raise ValueError("unsupported source pitch convention")
    monkeypatch.setattr("harmony_source.arrangement_pitch_metadata", unavailable)
    messages = stream(write_pack(tmp_path))
    info = next(msg for msg in messages if msg["type"] == "song_info")
    assert info["guide_fret_semantics"] == "relative"
    assert info["guide_open_midi"] is None
