 ...
 * --real-chart <private lead.json> --perf [--perf-only] [--profile]
...
let served = source;
const linkBuilder=served.match(/function hwyBuildLinkedTrailPaths\([^)]*\) \{/);
if (linkBuilder) served = once(served,linkBuilder[0],
    linkBuilder[0]+' if(window.__trailBuilds!==undefined)window.__trailBuilds++;');
...
add('out-and-back',base({notes:[note({sus:.8,sl:9,ln:true}),note({t:10.8,f:9,sus:.8,sl:5,ln:true}),note({t:11.6,sus:.4}),obstacle({t:10.9,f:9})]}),{image:true,seams:[[10,10.8],[10.8,11.6]],expectNarrow:true});
...
for(const technique of ['tr','vb'])add('linked-'+technique,base({notes:[note({sus:.77,ln:true,[technique]:true}),note({t:10.77,sus:1.23,[technique]:true}),obstacle({t:11})]}),{seams:[[10,10.77]],widthOnly:true,expectNarrow:true});
...
const holdAware=source.includes('function hwyBuildChordHoldGuidance(');
add('shared-hold-target',base({notes:[note({sus:2})],chords:[{t:10.2,id:0,notes:[note({t:10.2,s:1,sus:1.5}),note({t:10.2,s:2,f:7,sus:1.5})]}],chordTemplates:[{name:'Hold',frets:[-1,5,7,-1,-1,-1],fingers:[-1,1,3,-1,-1,-1]}]}),{image:true,expectNarrow:true,scales:[[10.1,.3],[10.8,holdAware?1:.3]],suppressedTrails:holdAware?[{t:10.2,s:1},{t:10.2,s:2}]:[]});
...
function validateScene(s,p){
    ...
    for(const [left,right] of s.seams||[]){const a=sourceStrands(p,left,ss).find(r=>Math.abs(r.samples.at(-1).t-right)<.0011),b=sourceStrands(p,right,ss)[0];check(a&&b,`${s.name}: missing connected seam ${left}-${right}`);if(!a||!b)continue;
        for(const kind of ['body','outline']){const x=dim(a.samples.at(-1),kind),y=dim(b.samples[0],kind);for(const key of s.widthOnly?['width','height']:['width','height','x','y','z'])check(Math.abs(x[key]-y[key])<2e-6,`${s.name}: ${kind} ${key} seam mismatch ${x[key]} vs ${y[key]}`);}}
    ...
    for(const [time,scale] of s.scales||[])check(Math.abs(widthScaleAt(p,time,ss)-scale)<.02,`${s.name}: expected scale ${scale} at ${time}, got ${widthScaleAt(p,time,ss)}`);
    for(const n of s.suppressedTrails||[])check(sourceStrands(p,n.t,n.s).length===0,s.name+': shared hold still has individual member trail');
...
async function main(){
    ...
    try{
        ...
        for(const s of scenes.filter(s=>!args.includes('--perf-only')&&(!option('--case')||option('--case').split(',').some(q=>s.name.includes(q))))){
        ...
        if(!option('--case')&&!args.includes('--perf-only')){
            ...
            results.push({name:'live-settings',samples:live});
            if(!reference){check(Math.abs(widthScaleAt(live[0],10.95)-.45)<.01,'live minimum scale not applied');check(Math.abs(widthScaleAt(live[1],10.95)-1)<.01,'live disabling retains narrowing');check(Math.abs(widthScaleAt(live[2],10.95)-1)<.01,'live lead-time change retains stale window');for(const p of live)check(p.modelBuilds===1,'settings unnecessarily rebuilt link topology');}
        }
        if(args.includes('--perf')){
            ...
            for(const s of perfScenes){await init(s.b,s.mode);const perf=await page.evaluate(()=>{const a=r.__trailAudit(),gl=a.ren.getContext(),extension=gl.getExtension('WEBGL_debug_renderer_info');for(let i=0;i<10;i++)r.draw(bundle);gl.finish();const cpu=[],finish=[];for(let round=0;round<3;round++){let t=performance.now();for(let i=0;i<10;i++)r.draw(bundle);cpu.push((performance.now()-t)/10);t=performance.now();gl.finish();finish.push(performance.now()-t);}return {cpuMsPerDraw:cpu,finishWaitMs:finish,gpu:extension?gl.getParameter(extension.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER),proof:__capture()};});results.push({name:'performance-'+s.name,...perf});console.log(s.name+' CPU '+JSON.stringify(perf.cpuMsPerDraw));
                if(args.includes('--profile')){
                    ...
                    await page.evaluate(()=>{for(let i=0;i<15;i++)r.draw(bundle);r.__trailAudit().ren.getContext().finish();});
                    const {profile}=await cdp.send('Profiler.stop');await cdp.detach();fs.writeFileSync(path.join(out,'profile-'+s.name+'.json'),JSON.stringify(profile));
                    ...
                    for(let i=0;i<profile.samples.length;i++){let id=profile.samples[i];const us=profile.timeDeltas[i],seen=new Set(),n=nodes.get(id),name=n.callFrame.functionName||'(anonymous)';self.set(name,(self.get(name)||0)+us);while(id){const node=nodes.get(id),key=node.callFrame.functionName||'(anonymous)';if(!seen.has(key)){inclusive.set(key,(inclusive.get(key)||0)+us);seen.add(key);}id=parents.get(id);}}
                    const rank=m=>Array.from(m,([name,us])=>({name,ms:Math.round(us/1000)})).sort((a,b)=>b.ms-a.ms).slice(0,20);
                    fs.writeFileSync(path.join(out,'profile-summary-'+s.name+'.json'),JSON.stringify({self:rank(self),inclusive:rank(inclusive)},null,2));
                }
            }
        }
