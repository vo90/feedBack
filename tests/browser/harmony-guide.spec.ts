...
async function installSongFixture(page: Page) {
      ...
      constructor(url: string) {
        ...
        setTimeout(() => {
          ...
          this.emit({
            ...
            has_keys: true, has_harmony: true,
          });
          ...
          this.emit({ type: 'keys', version: 1, data: [{ t: 0, key: 'Am', scale: 'natural_minor' }] });
          this.emit({ type: 'harmony', version: 1, data: [
...
test('local harmony edits reject conflicting scales, persist, and stay isolated to a song revision', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('.hg-source')).toHaveText('Your local guide');
