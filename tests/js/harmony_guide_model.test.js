...
test('key regions, equal-time replacements and user overrides resolve deterministically after seeks', async () => {
    ...
    assert.deepEqual(resolveHarmony(timeline, 2).source, { key: 'feedpak', harmony: 'local', scale: 'feedpak' });
...
test('generated tracks expire independently, unknown never becomes N.C., and seeks cannot carry stale chords', async () => {
    const { createHarmonyTimeline, resolveHarmony } = await model;
    const timeline = createHarmonyTimeline({ generated: {
        keys: { events: [{ t: 0, end: 20, key: 'C major' }] },
        scales: { events: [{ t: 0, end: 3, root: 'C', type: 'major' }, { t: 8, end: 12, root: 'D', type: 'dorian' }] },
        harmony: { events: [{ t: 1, end: 3, root: 'C', quality: 'maj' }, { t: 8, end: 10, root: 'D', quality: 'min' }] },
    } });
    const gap = resolveHarmony(timeline, 6);
    assert.equal(gap.key.label, 'C major');
    assert.equal(gap.scale, null);
    assert.equal(gap.current.status, 'unknown');
    assert.equal(gap.targetPc, null);
    assert.equal(gap.upcoming[0].label, 'Dm');
    const localScale = resolveHarmony(timeline, 9);
    assert.equal(localScale.key.root, 'C');
    assert.equal(localScale.scale.root, 'D');
    assert.equal(localScale.scale.degreeLabels[2], 'R');
    assert.equal(localScale.source.scale, 'charts');
    assert.equal(resolveHarmony(timeline, 2).current.label, 'C');
    assert.equal(resolveHarmony(timeline, 20).key, null);
    const wrap = resolveHarmony(timeline, 9, { loop: { start: 5, end: 10 } });
    assert.equal(wrap.upcoming[0].status, 'unknown');
    assert.equal(wrap.upcoming[0].t, 10);
});

test('local, authored and generated sources are selected per track, including explicit empty tracks', async () => {
    const { createHarmonyTimeline, resolveHarmony } = await model;
    const generated = { keys: [{ t: 0, key: 'Am' }], harmony: [{ t: 0, root: 'F', quality: 'maj' }],
        scales: [{ t: 0, root: 'D', type: 'dorian' }] };
    const options = { keys: [{ t: 0, key: 'C' }], generated };
    const independent = resolveHarmony(createHarmonyTimeline(options), 1);
    assert.equal(independent.key.label, 'C major');
    assert.equal(independent.scale.root, 'D');
    assert.equal(independent.current.label, 'F');
    assert.deepEqual(independent.source, { key: 'feedpak', harmony: 'charts', scale: 'charts' });
    const explicitScale = resolveHarmony(createHarmonyTimeline({ ...options, keys: [{ t: 0, key: 'C', scale: 'major' }] }), 1);
    assert.equal(explicitScale.scale.root, 'C');
    assert.equal(explicitScale.source.scale, 'feedpak');
    const cleared = resolveHarmony(createHarmonyTimeline({ ...options, overrides: { harmony: [], scales: [] } }), 1);
    assert.equal(cleared.key.root, 'C');
    assert.equal(cleared.current, null);
    assert.equal(cleared.scale, null);
    const deliberateUnknown = resolveHarmony(createHarmonyTimeline({ ...options, harmony: [{ t: 0, unknown: true }] }), 1);
    assert.equal(deliberateUnknown.current.status, 'unknown');
    assert.equal(deliberateUnknown.source.harmony, 'feedpak');
});

test('chart extensions validate their complete tones, while arbitrary authored tone overrides are ignored', async () => {
    const { createHarmonyTimeline, resolveHarmony } = await model;
    const event = { t: 0, root: 'G', quality: '7sus4', chord_tones: [7, 0, 2, 5], display_label: 'G7sus4' };
    const options = { keys: [{ t: 0, key: 'C' }], generated: { harmony: [event] } };
    const valid = resolveHarmony(createHarmonyTimeline(options), 1);
    assert.equal(valid.current.label, 'G7sus4');
    assert.equal(valid.current.supported, true);
    assert.equal(valid.scale.root, 'C');
    const incompatible = resolveHarmony(createHarmonyTimeline({ ...options, generated: { harmony: [{ ...event, chord_tones: [7, 0, 2, 6] }] } }), 1);
    assert.equal(incompatible.scaleStatus, 'conflict');
    const authored = resolveHarmony(createHarmonyTimeline({ keys: options.keys, harmony: [{ ...event, chord_tones: [1] }] }), 1);
    assert.deepEqual(authored.current.pitchClasses, [7, 0, 2, 5]);
});

test('physical capo markers agree with Queen capo-one voicing and do not transpose positive frets', async () => {
    const { makeScale, buildScaleFretboard } = await model;
    const board = buildScaleFretboard({ scale: makeScale('Ab', 'major'), targetPc: 8,
        openMidi: [41, 46, 51, 56, 60, 65], capo: 1, fretSemantics: 'physical' });
    const voicing = [[0, 4, 44], [1, 3, 48], [2, 0, 51], [3, 0, 56], [4, 0, 60], [5, 4, 68]];
    for (const [string, fret, midi] of voicing) assert.equal(board.markers.find(m => m.string === string && m.fret === fret)?.midi, midi);
    assert(board.markers.filter(m => m.fret > 0).every(m => m.fret > 1));
    assert(board.positions.every(p => p.spans.every(s => s.minFret > 1)));
});

test('an earlier bounded event does not expire a newer overlapping annotation', async () => {
    const { createHarmonyTimeline, resolveHarmony } = await model;
    const timeline = createHarmonyTimeline({ harmony: [{ t: 0, end: 8, root: 'C' }, { t: 4, end: 12, root: 'D' }] });
    assert.equal(resolveHarmony(timeline, 9).current.root, 'D');
    assert.equal(resolveHarmony(timeline, 12).current.status, 'unknown');
});
