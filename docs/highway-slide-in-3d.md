...
Linked trail paths retain the first member's visual start and sample each member's
incoming contour alongside slides and tremolo. Their shared visibility windows
still use the complete validated path. A continuation carrying an initial incoming
cue has an unknown origin, so that seam does not join the two physical paths;
authored attack suppression remains independent and unchanged. The event index
keeps hidden continuation heads out of gem narrowing while retaining their real
trails, including zero-sustain incoming approaches.
...
RS+ integration retains the thin trail style and technique ordering. Chords
with a validated fretted incoming cue keep individual member trails instead of
shared hold borders, including zero-sustain and internal incoming destinations.
Invalid incoming metadata and open-only destinations do not disable ordinary
shared holds. Position guides remain independent of this technique decision.
