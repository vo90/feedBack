...
### Notes
- **Stem visibility** → `noteStemVisible()` gates fretted connectors, chord/arpeggio drop lines and RS+ open/unpitched-muted stems. `belongsToBoxedChord` always suppresses stems, including after the flying box ends at onset; open arpeggio brackets do not count as boxes. Otherwise fretted notes use `noteStemsVisible` and open/muted bars use `openStringStemsVisible`, independently of notation style. Current open bars have rims, not vertical stems: preserve those rims. Both settings default on, persist through `_bgReadSetting`, and refresh through the flag-only setting listener. Keep fret labels, chord boxes and sustain trails independent. Pooled visibility and bounds must follow the actual visible stem when toggling live.
...
- **Chord frame-box** → the chord loop's `compactRepeatFrame`, `drawRsPlusChordFrame()` and `pRsChordFrame`. Current retains distance/repeat dimming. RS+ inspired uses pale neutral rims and a lightly transparent fill (`chordFrameGradTexRs`), stable approach opacity and stronger accent borders. Full chords use full-height panels; plain repeats use half-height panels. `chordBoxTop` defaults to `short-caps` (6% of frame width per upper corner); `full` restores the closed rim. Reset `uTopCap` on every pooled draw and mask both rim and halo so neither bridges the open middle. Fill and bottom/side edges stay unchanged. Explicit arpeggios use full-height purple side brackets with their original short caps; they always reset `uTopCap` to zero. `repeatChordFullBorder` defaults to false; when true, only `compactRepeatFrame` draws pass `fullTop` to close the rim and halo. Full-height technique repeats retain the selected caps. Both preferences are RS+-only, persist globally/per panel, and use the flag-only live listener. The repeated-chord checkbox is nested under Chord box top, disabled outside RS+ short-caps mode, and preserves its saved value when disabled.
