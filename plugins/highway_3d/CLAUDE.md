...
### Notes
- **RS+ stem visibility** → `noteStemsVisible` gates standalone fretted connectors and individual arpeggio/hand-shape drop lines. `openStringStemsVisible` gates the open/unpitched-muted outline mesh, together with `hasEnclosingChordFrame`. Both default on, persist through `_bgReadSetting`, and refresh through the flag-only setting listener. Keep fret labels, chord-member guides, sustain trails and Current appearance independent. Pooled visibility and bounds must follow the actual visible stem when toggling live.
