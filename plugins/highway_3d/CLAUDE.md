...
- **Technique markers** → `// ── Technique labels ──` in `drawNote()`. Current retains `triMat`, `bendChevronMat`, mute/harmonic factories and the tap geometry. RS+ inspired uses `rsPlusTechniqueFlags`, `rsPlusTechniqueCells`, `drawRsPlusTechniqueGlyph` and `rsPlusTechniqueMat`: cached 512px masks with contrasting sharp strokes and no baked glow. A combined mask gives each authored family a separate face cell. Masks become pooled planes through `_spriteMat2MeshMat`, which preserves their fog/tone-mapping policy. Bend chevrons follow `bendVisualDirY`; RS+ amount labels remain upright and show the exact chart semitone value. `bendSemisAtTime` still controls the bend path.
...
- **Chord frame-box** → the chord loop's `drawEdge`, `compactRepeatFrame`, and RS+ frame geometry/pool. Current retains distance/repeat dimming. RS+ inspired uses rounded edges, stable approach opacity, and stronger accent borders; its lightly transparent interior remains guidance rather than an opaque panel.
...
- **Repeat-chord detection** → `prevChordSig` / `prevChordTime` inside the chord loop. Same shape within 0.5 s may be a repeat, but `retainsChordGems` and technique-aware suppression preserve members needed to convey slides, bends, vibrato, harmonics and other techniques. Plain repeats use shorter frames. RS+ inspired does not dim a repeat just because it is a repeat.
...
- Keep glyph/artwork masks free of baked blur. `txtCache` owns the RS+ technique textures/materials; `_spriteMat2MeshMat` owns shared base conversions and its per-mesh clones. New style resources must be covered by pool reset and teardown paths.
...
- `settings.html` hydrates the style dropdown and changes descriptions of Vibrancy, Glow and Bloom to match the selected style. The Bloom title becomes **Soft glow** in RS+ inspired. Text size applies to text labels; RS+ face masks keep their fixed fit inside the note.
- Regression entry points: `highway_3d_rsplus_settings.test.js` (coercion, panel refresh, storage failure, UI hydration), `highway_3d_rsplus_techniques.test.js` (mask combinations, contrast, caching, fractional bends), the RS+ chord tests, and the existing live-settings/bend/repeat/trail suites. Screenshot and motion review are still required for proportions, layering, and readability.
...
Most text labels are rasterised by `txtMat(text, color, wide, style)`, with appearance driven by `TXT_STYLES`. **Do not edit the body of `txtMat()` to change a single label class** — change its preset. RS+ technique masks and exact bend-amount cues are separately cached by `rsPlusTechniqueMat()`; they do not inherit text-sprite shadows.
