...
In RS+ inspired, ordinary and accented notes have the same full body opacity throughout their approach. Gems, rims and technique symbols keep a fixed orientation as they travel down the highway, including chord and arpeggio notes. Bend and slide paths still follow the chart. Accents use a stronger rim; each chord member follows its own chart accent flag. Chords use pale neutral panels; plain repeats use half-height panels. By default, both use short rounded top caps with an open middle; **Chord box top → Full border** restores the closed outline. The optional **Full border on repeated chords** checkbox keeps only half-height repeat boxes closed while full-height chords retain short caps. Chords with techniques that need individual notes retain those notes and full-height frames. Hollow neck previews and purple arpeggio guidance remain visually separate from playable attacks.
...
| Note-gem stems | Shows or hides floor connectors beneath individual fretted notes, including arpeggio notes. Fret numbers and sustain trails keep their existing behavior. Defaults to on. |
| Chord box top | **Short caps** (default) keeps 6% of the frame width at each upper corner and leaves the middle open. **Full border** restores the original closed top. Applies only to RS+; fill, side/bottom edges and arpeggio guidance keep their existing behavior. |
| Full border on repeated chords | Off by default. With Short caps selected, closes only half-height repeat boxes, including compact palm-muted repeats. Full-height chords and technique repeats retain short caps. The checkbox is disabled with Full border or Current notation selected, and keeps its saved choice. |
| Open-string stems | Shows or hides stems beside open-string and unpitched muted-note bars. Bars inside ordinary chord boxes still omit their stems. Defaults to on. |
...
Both stem switches live under **Note appearance**, apply only to RS+ inspired, and update immediately. The split-screen panel's **3D settings** expose notation style, both stem switches, Glow, and Soft glow / bloom independently for each panel. **Chord box top (RS+)** is available beside the stem controls in Note appearance and in each split-screen panel. Its repeated-chord sub-option is available there too. Both saved choices update immediately and are retained when switching notation styles. Global settings apply wherever no panel override is saved.

The note and technique vocabulary follows [Ubisoft's RS+ notehead guide](https://www.ubisoft.com/en-gb/game/rocksmith/plus/news-updates/11LCT7xGpOMZrwjrRMbZbS/rocksmith-notehead-guide). This is an original visual approximation with deliberate readability choices, including no approach-distance dimming of playable notes. It does not change chart data, timing or scoring. Gems meet the trail at its resolved initial pitch. The bend-start rules below distinguish delayed ordinary bends from explicit pre-bends.

Bends use one chevron for a half step (one semitone), two for a whole step, and more for larger bends, following the examples in [Ubisoft's Rocksmith 2014 notehead guide](https://static2.cdn.ubi.com/ncsa/rocksmith/website/guides/Notehead-tail%20guide.pdf). The count uses the chart's peak bend amount, including authored curves, with the existing rounded 1–4 display range; fractional values still drive the exact bend curve. Each arrow keeps its string-colored fill with a thin pale edge and dark contour. Stacks grow away from the gem without shrinking individual arrows or adding an amount label.

Open strings and unpitched muted notes use colored bars with an optional pale stem to the highway floor rather than a stretched gem outline. With **Open-string stems** enabled, an approaching ordinary chord box replaces the stem; if the note remains visible after the box disappears at the play line, its full floor stem returns. Standalone bars retain their full stems even when associated with hand-shape or purple arpeggio guidance; there is no short-tick variant. Disabling the switch hides these stems throughout the note's lifetime. Sustains have narrower colored strokes; full-height purple side brackets with short caps identify arpeggio guidance. With no saved arrow preference, RS+ slides use the trail alone. Explicitly enabled slide arrows remain available, and switching to Current restores its usual defaults.

Pale face symbols have a narrow dark contour so they stay distinct on bright string colors. Natural and pinch harmonics keep their hollow ring shapes with stronger strokes. Combined technique marks retain their proportions and share the face without overlapping or clipping. Palm-mute marks keep their pale edge and string-dark center; bend and slide arrows keep their string color. These masks, including complete bend-arrow stacks, remain sharp with Glow at zero and add no rendering passes.
