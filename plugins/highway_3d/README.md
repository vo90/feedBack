...
Both stem switches live under **Note appearance**, apply only to RS+ inspired, and update immediately. The split-screen panel's **3D settings** expose notation style, both stem switches, Glow, and Soft glow / bloom independently for each panel. **Chord box top (RS+)** is available beside the stem controls in Note appearance and in each split-screen panel. Its saved choice updates immediately and is retained when switching notation styles. Global settings apply wherever no panel override is saved.
